<script lang="ts">
  ...
  const yGet = (d: EquityPoint) => Math.max(d.chameleon_pct, d.control_pct);
  const yGetMin = (d: EquityPoint) => Math.min(d.chameleon_pct, d.control_pct);

  function linePath(
    data: EquityPoint[],
    xScale: (d: Date) => number,
    yScale: (n: number) => number,
    key: 'chameleon_pct' | 'control_pct'
  ) {
    return data
      .map((d, i) => {
    ...
    <div class="h-72">
      {#if filtered.length > 1}
        <LayerCake
          ...
          y={yGet}
          yDomain={[
            Math.min(...filtered.map(yGetMin), 0) - 2,
            Math.max(...filtered.map(yGet)) + 2
          ]}
          xScale={scaleTime()}
          padding={{ top: 10, right: 10, bottom: 24, left: 36 }}
        >
            ...
            <Svg>
              ...
              <g class="ticks-y">
                {#each ticks as t (t)}
                  ...
                  <text x="-6" y={yScale(t)} dy="0.32em" text-anchor="end" fill="#64748b" font-size="10">{t}%</text>
                {/each}
              </g>
              <path d={linePath(filtered, xScale, yScale, 'control_pct')} fill="none" stroke="#6366f1" stroke-width="2" />
              <path d={linePath(filtered, xScale, yScale, 'chameleon_pct')} fill="none" stroke="#10b981" stroke-width="2" />
            </Svg>
          {/snippet}
        </LayerCake>
      {:else}
